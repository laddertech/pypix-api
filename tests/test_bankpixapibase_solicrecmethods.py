...
def dummy_bank_pix_api():
    api = DummyBankPixAPIBaseSolicRec()
    api.session = MagicMock()
    return api

def test_criar_solicrec(dummy_bank_pix_api):
    dummy_bank_pix_api.session.post.return_value = MagicMock(
        json=lambda: {"result": "ok"}, raise_for_status=lambda: None
    )
    body = {"calendario": {"expiracao": 3600}, "devedor": {"cpf": "12345678909", "nome": "Fulano"}}
...
def test_consultar_solicrec(dummy_bank_pix_api):
...
def test_revisar_solicrec(dummy_bank_pix_api):
