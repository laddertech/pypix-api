...
def test_consultar_pix_bb_basico(bb_pix_api: BBPixAPI) -> None:
    bb_pix_api.session.get.return_value = MagicMock(
        json=lambda: {
            'parametros': {
    ...
    bb_pix_api.session.get.assert_called_once()
    args, kwargs = bb_pix_api.session.get.call_args
    assert '/pix-bb' in args[0]
