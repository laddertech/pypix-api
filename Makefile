...
security-scan: ## Executar scan completo de segurança
	...
	uv run bandit -r pypix_api/ -ll -f json -o bandit-report.json || true
	@echo "2. Safety - Vulnerability database"
	uv pip install --quiet safety
...
security-report: security-scan ## Gerar relatório de segurança consolidado
	...
	@uv run python -c "import json, os, datetime; print('🔒 Relatório de segurança gerado em:', datetime.datetime.now())"
...
benchmark-report: ## Gerar relatório de benchmark
	uv run pytest tests/benchmarks --benchmark-only --benchmark-json=benchmark-report.json
